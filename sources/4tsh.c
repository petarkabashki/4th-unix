...
| Added Francois Perrad LINT patches               | Francois Perrad  |221016|
| Fixed bug in JSLEEP entry of Interpret()         | J.L. Bezener     |140920|
+--------------------------------------------------+------------------+------+
...
+--------------------------------------------------+------------------+------+
  ...
  Copyright (C) 2007, 2020 J.L. Bezemer
...
static Job Process [MAXJOBS];          /* job table */
static int ToS = INIT;                 /* top of job stack */
  ...
  if (CHECK (a = from_4th (obj, stk)))  /* get pid */
     if (CHECK (b = from_4th (obj, stk)))
        if ((b = ToJob (b)) < ToS)      /* convert, if valid decompile */
           (void) dump_4th (Process [b].Job, stdout, a, a + MAXDUMP);
}
