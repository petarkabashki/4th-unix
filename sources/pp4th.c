...
| Added -n switch in DecodeOptions()               | J.L. Bezemer     |291017|
| Corrected usage message                          | J.L. Bezemer     |111217|
| Added FTRAN messages to Prep4tH()                | J.L. Bezemer     |020520|
| Turbo C needs a smaller PP4tH, pp4th86.h         | J.L. Bezemer     |060520|
| Added __MINGW32__ to DOSALIKE macro              | J.L. Bezemer     |160520|
| Added __POCC__ to DOSALIKE macro                 | J.L. Bezemer     |140920|
+--------------------------------------------------+------------------+------+
...
+--------------------------------------------------+------------------+------+
  ...
  Copyright (C) 2014, 2020 J.L. Bezemer
...
#endif
                                       /* global variables */
static char   **PArgs   = NULL;        /* preprocessor argv */
static int      PArgn   = 0;           /* preprocessor argc */
static short    Cmds    = 0;           /* issued commands */
static char    *PpFile  = NULL;        /* holds the temporary filename */
static char    *OutFile = NULL;        /* holds the output filename */
static char    *InFile  = NULL;        /* holds the input filename */
static Hcode   *Object  = NULL;        /* compiled program */

int   optind  = 0;                     /* argv index after getopt() */
char *optarg;                          /* argument pointer */

#if defined (_WIN32) || defined (__TURBOC__) || defined (__DJGPP__) || defined (__OS2__) || defined (__MSDOS__) || defined (__MINGW32__) || defined (__POCC__)
#define DOSALIKE
#define ISDIR(a) (((a) == '/') || ((a) == '\\'))
#else
#define ISDIR(a) ((a) == '/')
...
{                                      /* array with error messages */
  static const char* const Msg [] =
  {
    ...
    "%s\tCannot %s file \'%s\'\n",
    "%s\tResult: %ld\n",
    "%s\tNo object compiled\n",
    "4tH library v%x.%x - Copyright 1994,2020 J.L. Bezemer\n",
    ...
    "%s\t%s\n",
    "%s\tOut of memory\n",
    "Usage: pp4th [-Vvicsgdxpkn] [-D symbol] [-o outfile] infile [arguments]\n"
  };
...
{
  static const char* const Msg [] =    /* the preprocessor messages */
  {
    "No errors",
    "Bad number",
...

{
  ...
  while ((c = GetOpt (Argc, Argv, "VvpkincdsxgD:o:")) != -1)
    switch (c)
      {
        ...
        case ('n'): Enable (CMD_PREP); PArgs [PArgn++] = "-n"; break;
