...
3 PRINT:PRINT:PRINT
4 Z = ASK("DO YOU WANT INSTRUCTIONS? ")
6 N=8
10 IF PEEK(Z,0)=ORD("n") THEN 47
11 IF COMP(Z, "AGAIN")=0 THEN 1960
...
44 PRINT "YOUR GOAL IS TO COMPLETE YOUR ";N;" YEAR TERM OF OFFICE."
46 PRINT "GOOD LUCK!"
47 PRINT : Z = 0
50 A=(60000+(RND(1000))-(RND(1000)))
55 B=(500+(RND(10))-(RND(10)))
65 D=2000
100 W=(RND(10)+95)
102 PRINT
105 PRINT "YOU NOW HAVE ";A;" RALLODS IN THE TREASURY."
110 PRINT (B);" COUNTRYMEN, ";
115 Y=(((RND(10)/2)+10))
120 IF C=0 THEN 140
130 PRINT (C);" FOREIGN WORKERS, ";
140 PRINT "AND ";(D);" SQ. MILES OF LAND."
150 PRINT "THIS YEAR INDUSTRY WILL BUY LAND FOR ";W;
152 PRINT " RALLODS PER SQUARE MILE."
155 PRINT "LAND CURRENTLY COSTS ";Y;" RALLODS PER SQUARE MILE TO PLANT."
162 PRINT
200 INPUT "HOW MANY SQUARE MILES DO YOU WISH TO SELL TO INDUSTRY? ";H
210 IF H<0 THEN 200
220 IF (H-1)<D-1000 THEN 300
...
300 D=(D-H)
310 A=(A+(H*W))
320 INPUT "HOW MANY RALLODS WILL YOU DISTRIBUTE AMONG YOUR COUNTRYMEN? ";I
342 IF I<0 THEN 320
350 IF I<A THEN 400
360 IF I=A THEN 380
...
399 GOTO 1000
400 A=(A-I)
410 INPUT "HOW MANY SQUARE MILES DO YOU WISH TO PLANT? ";J
421 IF J<0 THEN 410
422 IF J-1<B*2 THEN 426
423 PRINT "   SORRY, BUT EACH COUNTRYMAN CAN ONLY PLANT 2 SQ. MILES."
424 GOTO 410
426 IF J-1<D-1000 THEN 430
...
1010 A=(A-K)
1020 E=A
1100 IF (I/100-B)>-1 THEN 1120
1105 IF I/100<50 THEN 1700
1110 PRINT (B-(I/100));" COUNTRYMEN DIED OF STARVATION"
1120 F=((2000-D)/RND(1000))
1122 IF K<25 THEN 1130
1125 F=(F/(K/25))
1130 IF F<1 THEN 1150
1140 PRINT F;" COUNTRYMEN DIED OF CARBON-MONOXIDE AND DUST INHALATION"
1150 IF ((I/100)-B)<0 THEN 1170
1160 IF F>0 THEN 1180
1165 GOTO 1200
1170 PRINT "   YOU WERE FORCED TO SPEND ";((F+(B-(I/100)))*9);
1172 PRINT " RALLODS ON FUNERAL EXPENSES"
1174 L=(F+(B-(I/100)))
1175 A=(A-((F+(B-(I/100)))*9))
1176 GOTO 1185
1180 PRINT "   YOU WERE FORCED TO SPEND ";(F*9);" RALLODS ON ";
1181 PRINT "FUNERAL EXPENSES."
1182 L=F
1183 A=(A-(F*9))
1185 IF A>-1 THEN 1194
1187 PRINT "   INSUFFICIENT RESERVES TO COVER COST - LAND WAS SOLD"
1189 D=(D+(A/W))
1190 A=0
1194 B=(B-L)
1200 IF H=0 THEN 1250
1220 R=(H+(RND(10))-(RND(20)))
1224 IF C>0 THEN 1230
1226 R=R+20
1230 PRINT R;" WORKERS CAME TO THE COUNTRY AND";
1250 P=(((I/100-B)/10)+(K/25)-((2000-D)/50)-(F/2))
1255 PRINT ABS(P);" COUNTRYMEN ";
1260 IF P<0 THEN 1275
1265 PRINT "CAME TO";
1270 GOTO 1280
1275 PRINT "LEFT";
...
1290 B=(B+P)
1292 C=(C+R)
1305 U=(((2000-D)*((RND(10)+15)/20)))
1310 IF C=0 THEN 1324
1320 PRINT "OF ";(J);" SQ. MILES PLANTED,";
1324 IF J>U THEN 1330
1326 U=J
1330 PRINT " YOU HARVESTED ";(J-U);" SQ. MILES OF CROPS."
1340 IF U=0 THEN 1370
1344 IF T>1 THEN 1370
1350 PRINT "   (DUE TO ";
1355 IF T=0 THEN 1365
1360 PRINT "INCREASED ";
1365 PRINT "AIR AND WATER POLLUTION FROM FOREIGN INDUSTRY.)"
1370 Q=((J-U)*(W/2))
1380 PRINT "MAKING ";(Q);" RALLODS."
1390 A=(A+Q)
1400 V=(((B-P)*22)+(RND(500)))
1405 O=((2000-D)*15)
1410 PRINT " YOU MADE ";V-O;" RALLODS FROM TOURIST TRADE."
1420 IF O=0 THEN 1450
1425 IF V-O>Z-1 THEN 1450
1430 PRINT "   DECREASE BECAUSE ";
1435 G=RND(10)
1440 IF G<3 THEN 1460
1442 IF G<5 THEN 1465
1444 IF G<7 THEN 1470
1446 IF G<9 THEN 1475
1448 IF G<11 THEN 1480
1450 Z=(V-O)
1451 A=(A+Z)
1452 GOTO 1500
...
1482 GOTO 1450
1500 IF L>200 THEN 1600
1505 IF B<343 THEN 1700
1510 IF (E/100)>5 THEN 1800
1515 IF C>B THEN 1550
1520 IF N-1=S THEN 1900
1545 GOTO 2000
1550 PRINT
1552 PRINT
...
1925 PRINT "YOU HAVE SUCCESFULLY COMPLETED YOUR ";N;" YEAR TERM"
...
1961 INPUT S
1962 IF S<0 THEN 1590
1963 IF S<8 THEN 1969
1965 PRINT "   COME ON, YOUR TERM IN OFFICE IS ONLY ";N;" YEARS."
1967 GOTO 1960
